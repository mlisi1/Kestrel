...
import numpy as np
import torch
from utils.colmap import rotmat2qvec, qvec2rotmat
...
class Gaussian:
    ...
    features_rest: Union[np.ndarray, torch.Tensor]  # ndarray[n, 3, 15], or tensor[n, 15, 3]
    ...
    def load_from_ply(cls, path: str, sh_degrees: int = -1):
        ...
        features_rest = cls.load_array_from_plyelement(
            plydata.elements[0], "f_rest_"
        ).reshape((xyz.shape[0], 3, -1))
        if sh_degrees >= 0:
            assert features_rest.shape[-1] == (sh_degrees + 1) ** 2 - 1
        else:
    ...
    def load_from_state_dict(cls, sh_degrees: int, state_dict: dict, key_prefix: str = "gaussian_model._"):
        init_args = {"sh_degrees": sh_degrees}
        ...
        if f"{key_prefix}features_extra" in state_dict:
            ...
            init_args["real_features_extra"] = torch.empty(
                (init_args["xyz"].shape[0], 0), device=init_args["xyz"].device
            )
...
class GaussianTransformUtils:
    ...
    def rotate_by_euler_angles(cls, xyz, rotation, x: float, y: float, z: float):
        ...
        xyz, rotation = cls.rotate_by_matrix(xyz, rotation, rotation_matrix.to(xyz))
        return xyz, rotation
    ...
    def rotate_by_wxyz_quaternions(cls, xyz, rotations, features, quaternions: torch.tensor):
        if torch.all(quaternions == 0.) or torch.all(quaternions == torch.tensor(
                [1., 0., 0., 0.], dtype=quaternions.dtype, device=quaternions.device)):
            return xyz, rotations, features

        rotation_matrix = torch.tensor(
            qvec2rotmat(quaternions.cpu().numpy()), dtype=torch.float, device=xyz.device
        )
        xyz = torch.matmul(xyz, rotation_matrix.T)
        rotations = torch.nn.functional.normalize(cls.quat_multiply(rotations, quaternions))

        if features.shape[1] > 1:
            features = features.clone()
            degree_1 = features[:, 1:4, :].transpose(1, 2)
